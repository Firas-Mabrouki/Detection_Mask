...
from voiceAssistance import text2speachEN
...
def show_frame():
    ...
    cv2image=cv2.resize(cv2image, (1280,720))
       ...
       if(distanceFromCam>50 and distanceFromCam<75):
           ...
           if(mask==True and secondCheck==''):
               ...
               if(who==False):
                   ...
                   title="WELCOME MR "+name.upper()+", WEAR MASK TO OPEN THE DOOR"
                   #text2speachEN(title)
                   secondCheck=who
               ...
               if(dateEntry1==''):
                   ...
                   title='DOOR IS OPEN, MR '+name.upper()+' PLEASE ENTER !'
                   #text2speachEN(title)
                   secondCheck = ''
               elif (dateEntry1!='' and dateExit1==''):
                   markAttendance(db, secondCheck)
                   title='DOOR IS OPEN, SEE YOU MR '+name.upper()
                   #text2speachEN(title)
                   secondCheck = ''
               elif (dateEntry1!='' and dateExit1!='' and dateEntry2==''):
                   markAttendance(db, secondCheck)
                   title='DOOR IS OPEN, WELCOME BACK '+name.upper()
                   #text2speachEN(title)
                   secondCheck = ''
               elif(dateEntry1!='' and dateExit1!='' and dateEntry2!='' and dateExit2==''):
                   markAttendance(db, secondCheck)
                   title='DOOR IS OPEN, GOODBYE MR '+name.upper()
                   #text2speachEN(title)
                   secondCheck = ''
           elif (mask==False and secondCheck != ''):
               name=db.child("Employees/"+secondCheck+"/name").get().val()
               title="MR "+name.upper()+", PLEASE WEAR MASK TO OPEN THE DOOR"
               #text2speachEN(title)
               chance=chance+1
...
def GetUpdatedModelThread():
    ...
    while True:
      ...
      #guest=db.child("sync/guest").get().val()
      if(chance==2):
          secondCheck=''
          title="WELCOME TO INNOVUP ATTENDANCE SYSTEM"
          chance=0
...
'''
      if(guest==1):
          firebase=pyrebase.initialize_app(firebaseConfig)
          storageGuest = firebase.storage()
          _,frame=cap.read()
          rightNow=datetime.now().strftime('%Y_%m_%d in %H_%M_%S')
          cv2.imwrite('{}.jpg'.format(rightNow), frame)
          storageGuest.child('Guests/{}.jpg'.format(rightNow)).put('{}.jpg'.format(rightNow))
          print('Guest image saved !!')
          db.child('sync').update({"guest":0})
          os.remove('{}.jpg'.format(rightNow))
'''
...
if __name__=='__main__':
    while connect()==False:
        #text2speachEN('PLEASE CHECK THE INTERNET CONNECTION !')
    ...
    tApp.join()
    tUpdate.join()
    #print("DEVICE WILL SHUTDOWN AFTER 10 SECONDS")
    #sleep(10)
